...
bool HelloWorld::init()
{
    ...
    BigFishSprite *bigFishSprite    = BigFishSprite::createWithKey(kBIG_FISH_YELLOW);
    bigFishSprite->setPosition(ccp(visibleSize.width + 102., visibleSize.height/2));
//    bigFishSprite->setPosition(ccp(visibleSize.width/2, visibleSize.height/2));
...
#define COLLIPSE_RANGE  30.
    ...
    if (this->displayPearls->count() > 0) {
        // 가까운 볼을 탐색한 후 먹자.
        BigFishSprite *bigFishSprite    = (BigFishSprite*)this->getChildByTag(TAG_BIG_FISH_SPRITE);
        ...
        for (int i=0; i < this->displayPearls->count(); i++) {
            ...
            CCSprite *pearlSprite   = (CCSprite*)this->displayPearls->objectAtIndex(i);
            float tmpDistance    = ccpDistance(bigFishSprite->getPosition(), pearlSprite->getPosition());
...
//                BigFishSprite *enumBigFishSprite    = (BigFishSprite*)deletePearls->objectAtIndex(i);
...
////        float anger
//        float targetDistanceX    = nearestPearSprite->getPositionX() - bigFishSprite->getPositionX();
//        float targetDistanceY    = nearestPearSprite->getPositionY() - bigFishSprite->getPositionY();
//        
//        float targetDistance    = nearestPearSprite->getPosition().getDistance(bigFishSprite->getPosition());
//        float goX   = 0.;
//        float goY   = 0.;
...
void HelloWorld::callFish() {
    ...
    if (this->isLeft == false) {    // 왼쪽으로 갈 경우
        this->goPoint(ccp(-1*BIG_FISH_MARGIN_X, visibleSize.height/2), false, (float)TERM_FISH_CROSS_SECOND);
    } else {
        this->goPoint(ccp(visibleSize.width+BIG_FISH_MARGIN_X, visibleSize.height/2), false, (float)TERM_FISH_CROSS_SECOND);
    }
}
...
bool HelloWorld::ccTouchBegan(cocos2d::CCTouch *touch, cocos2d::CCEvent *pEvent) {
...
//    BigFishSprite *bigFishSprite    = (BigFishSprite*)this->getChildByTag(TAG_BIG_FISH_SPRITE);
//    bigFishSprite->stopAllActions();
//    float deltaAngle = pt.getAngle(bigFishSprite->getPosition())*180/3.141592+270.;
//    CCLog("deltaAngle = %f, bigFishRotation = %f", deltaAngle, bigFishSprite->getRotation());
//    
//    bigFishSprite->runAction(CCSpawn::create(
//                                             CCRotateTo::create(.3, deltaAngle),
...
void HelloWorld::ccTouchCancelled(cocos2d::CCTouch *touch, cocos2d::CCEvent *pEvent) {
