...
BigFishSprite *BigFishSprite::createWithKey(eBigFishKey k) {
    ...
    if (pob->initWithFile("fish2.png", CCRect(k * TEXTURE_WIDTH, 0.,
                                              TEXTURE_WIDTH, TEXTURE_HEIGHT))) {
        pob->autorelease();
        pob->setAnchorPoint(
            ccp(MOUSE_POINT_X / TEXTURE_WIDTH, MOUSE_POINT_Y / TEXTURE_HEIGHT));
    } else {
        delete pob;
        pob = NULL;
    }
...
void BigFishSprite::moveBy(CCPoint point) {
    ...
    CCPoint nextPoint =
        ccp(this->getPositionX() + point.x, this->getPositionY() + point.y);

    CCPoint diffPoint = ccpSub(nextPoint, prevPoint);
    //    CCPoint diffPoint = ccp(nextPoint.x - prevPoint.x, prevPoint.y -
    //    nextPoint.y);
...
void BigFishSprite::animating() {
    CCTexture2D *bigFishTexture =
        CCTextureCache::sharedTextureCache()->addImage("fish2.png");
    CCArray *bigFishFrames = CCArray::create();
    bigFishFrames->retain();

    for (int i = 0; i < 4; i++) {
        CCSpriteFrame *bigFishFrame = CCSpriteFrame::createWithTexture(
            bigFishTexture,
            CCRect(0., TEXTURE_HEIGHT, TEXTURE_WIDTH, TEXTURE_HEIGHT));
        bigFishFrames->addObject(bigFishFrame);
    }

    CCAnimation *bigFishAnimation =
        CCAnimation::createWithSpriteFrames(bigFishFrames, .2);  // 1초

    this->_state = kBIG_FISH_EATING;

    this->runAction(
        CCRepeatForever::create(CCAnimate::create(bigFishAnimation)));

    //    this->runAction(CCSequence::create(
    //                                       CCMoveBy::create(3.,
    //                                       ccp(TEXTURE_WIDTH*2+480., 0.))
    //                                       ,CCDelayTime::create(2.)
    //                                       ,CCCallFunc::create(this,
    //                                       callfunc_selector(BigFishSprite::close))
    //                                       ,NULL
    //                                       ));
}
...
void BigFishSprite::eating() {
    if (this->_state == kBIG_FISH_READY) {
        ...
        for (int i = 0; i < 4; i++) {
            CCSpriteFrame *bigFishFrame = CCSpriteFrame::createWithTexture(
                bigFishTexture, CCRect(offsetRect.origin.x, TEXTURE_HEIGHT * i,
                                       TEXTURE_WIDTH, TEXTURE_HEIGHT));
            bigFishFrames->addObject(bigFishFrame);
        }
        ...
        this->_state = kBIG_FISH_EATING;
        //        CCAnimation *blingBlingAnimation   =
        //        CCAnimationCache::sharedAnimationCache()->animationByName("clam_animation");
        this->runAction(CCSequence::create(
...
void BigFishSprite::flipX(bool b) {
    if (b == false) {
        ...
        this->setAnchorPoint(
            ccp((TEXTURE_WIDTH - MOUSE_POINT_X) / TEXTURE_WIDTH,
                MOUSE_POINT_Y / TEXTURE_HEIGHT));
    }
    this->setFlipX(b);
}